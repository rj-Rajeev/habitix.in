...
export default function CourseDetail({ slug }: { slug: string }) {
  ...
  async function enroll() {
    ...
    try {
      await enrollInCourse(data.course._id, data.course.title);
    ...
    <>
      ...
      {contentsOpen && <div className="fixed inset-0 z-50 lg:hidden">
        ...
        <aside className="absolute right-0 top-0 flex h-full w-[min(88vw,360px)] flex-col bg-surface shadow-[var(--shadow-md)] animate-[course-drawer-in_220ms_ease-out] motion-reduce:animate-none" role="dialog" aria-modal="true" aria-label="Course contents">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto"><CourseSidebar modules={modules} activeLessonId={activeLesson?._id} expandedModules={expandedModules} setExpandedModules={setExpandedModules} completedLessons={completedLessons} onSelect={selectLesson} progress={progress} hasPaidAccess={hasAccess} showHeading={false} /></div>
        </aside>
      </div>}
      </main>
    </>
...
function LessonContent({ lesson, hasAccess, onEnroll, enrolling }: { lesson: Lesson; hasAccess: boolean; onEnroll: () => void; enrolling: boolean }) {
  const previewContent = lesson.markdownContent || lesson.description || "This lesson is ready to explore. Enroll to access the complete lesson.";
  if (!hasAccess) return <article className="lesson-prose"><div className="relative h-[420px] overflow-hidden"><div className="relative z-0">{lesson.markdownContent ? <MarkdownContent content={previewContent} /> : <p className="text-base leading-8 text-text-secondary">{previewContent}</p>}</div><div className="absolute inset-x-0 bottom-0 z-20 px-4 pb-1 pt-24"><div className="pointer-events-none absolute inset-x-0 bottom-0 h-56 bg-gradient-to-t from-background via-background/95 to-transparent" /><div className="relative rounded-container border border-border bg-surface p-5 shadow-[var(--shadow-sm)]"><LockKeyhole className="h-5 w-5 text-text-muted" aria-hidden="true" /><h3 className="mt-3 text-base font-semibold text-text-primary">Continue with enrollment</h3><p className="mt-1 text-sm leading-6 text-text-secondary">Enroll to access the full lesson and learning materials.</p><button type="button" onClick={onEnroll} disabled={enrolling} className="ui-button mt-4" data-variant="primary">{enrolling ? "Preparing enrollment..." : "Enroll now"}</button></div></div></div>{lesson.videoUrl && <div className="mt-8 flex items-center gap-2 rounded-control border border-border bg-surface-subtle px-4 py-3 text-sm text-text-secondary"><Play className="h-4 w-4" aria-hidden="true" /><span>Video lesson</span><LockKeyhole className="h-4 w-4" aria-hidden="true" /><span>Available after enrollment</span></div>}{lesson.pdfUrl && <div className="mt-8 flex items-center gap-2 rounded-control border border-border bg-surface-subtle px-4 py-3 text-sm text-text-secondary"><FileText className="h-4 w-4" aria-hidden="true" /><span>PDF resource</span><LockKeyhole className="h-4 w-4" aria-hidden="true" /><span>Available after enrollment</span></div>}</article>;
