...
type LessonForm = { title: string; description: string; order: number; markdownContent: string; videoUrl: string; pdfUrl: string };

const blankModule: ModuleForm = { title: "", description: "", order: 0 };
const blankLesson: LessonForm = { title: "", description: "", order: 0, markdownContent: "", videoUrl: "", pdfUrl: "" };
...
export default function CourseBuilder({ courseId }: { courseId: string }) {
  ...
  function editLesson(moduleId: string, lesson: Lesson) { setEditingLesson({ moduleId, lessonId: lesson._id }); setLessonForm({ moduleId, values: { title: lesson.title, description: lesson.description ?? "", order: lesson.order, markdownContent: lesson.markdownContent ?? "", videoUrl: lesson.videoUrl ?? "", pdfUrl: lesson.pdfUrl ?? "" } }); }
  ...
  return <AppShell eyebrow="Admin" title="Course builder" action={<Link href="/admin/courses" className="inline-flex h-10 items-center gap-2 rounded-full border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700"><ArrowLeft className="h-4 w-4" /> Courses</Link>}>
    <div className="space-y-5">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-semibold text-slate-950">{course.title}</h2><Status status={course.status} /></div><p className="mt-1 text-sm text-slate-500">{course.price === 0 ? "Free" : course.price} · /{course.slug}</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setEditingCourse((value) => !value)} disabled={pending !== null} className="inline-flex min-h-9 items-center gap-1 rounded-lg border border-slate-200 px-3 text-xs font-semibold text-slate-700 disabled:opacity-50"><Pencil className="h-3.5 w-3.5" /> Edit details</button><button type="button" onClick={() => void togglePublish()} disabled={pending !== null} className="inline-flex min-h-9 items-center gap-1 rounded-lg bg-slate-950 px-3 text-xs font-semibold text-white disabled:opacity-50">{pending === "publish" && <Loader2 className="h-3.5 w-3.5 animate-spin" />}{course.status === "published" ? "Unpublish" : "Publish"}</button></div></div>
